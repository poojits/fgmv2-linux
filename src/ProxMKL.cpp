									 ...
									 double *b, double *upd_t, double fv,double lambda,double &f_w_, double f_omega)
{
	...
	for(i=0; i<40; i++)
	{
     ...
     /*  pred_temp1 = 0;
		...
		pred_temp1 = C*pred_temp1;
		if(fabs(pred_temp1-pred)>1e-4) //Changed to fabs by Poojt
		{
			printf("The difference happened\n");
		}*/
		btot = 0;
	...
	for(t = 0; t < numEpochs; t++)
	{
	  ...
	  // t_finish = clock();
	   ...
	   //printf("line_search elaps time is %f\n",run_time);
		if (fabs(f_w_temp-f_w)<eps_inner*fabs(f_w_temp)&&t>1) //Changed to fabs by Poojt
		{
			break;
		}else
		{
	...
	for(t = 0; t < numEpochs; t++)
	{
	  ...
	  // t_start = clock();
	   ...
	   //printf("line_search_logistic elaps time is %f\n",run_time);
		if (fabs(f_w_temp-f_w)/fabs(f_w_temp)<eps_inner&&t>1) //Changed to fabs by Poojt
		{
			break;
		}else
		{
